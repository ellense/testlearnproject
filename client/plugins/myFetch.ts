...
declare global {
  ...
  function $Authorization(options: RequestOptions): Promise<ITokens | null>;
  function $RefreshToken(): Promise<ITokens | string | undefined>;
}

export default defineNuxtPlugin(() => {
  const config = useRuntimeConfig().public.API as PublicApi;
  const ls: Storage = localStorage;
  const token = reactive<Partial<ITokens>>({
    access: ls.getItem(config.ACCESS_KEY) || undefined,
    refresh: ls.getItem(config.REFRESH_KEY) || undefined,
  });

  const setToken = (data: ITokens) => {
  ...
  globalThis.$fetch = $fetch.create({
    ...
    async onResponse({ request, options, response }) {
      ...
      if (response.status === 401 && isBearer) {
        ...
        try {
          response._data = await $fetch(request, options as NitroFetchOptions<NitroFetchRequest>);
          return;
        } catch (e) {
          console.log("MyFetch__", e);
        }
      }
  ...
  globalThis.$Post = async <T>(apiUrl: string, options: RequestOptions): Promise<T> => {
  ...
  globalThis.$Get = async <T>(apiUrl: string, options: RequestOptions): Promise<T> => {
    return await $fetch(apiUrl, {
      method: "GET",
      params: { ...options.params },
      isBearer: options.isBearer,
  ...
  globalThis.$Delete = async <T>(apiUrl: string, options: RequestOptions): Promise<T> => {
    return await $fetch(apiUrl, {
      method: "DELETE",
      body: { ...options.data },
      params: { ...options.params },
  ...
  globalThis.$Put = async <T>(apiUrl: string, options: RequestOptions): Promise<T> => {
    return await $fetch(apiUrl, {
      method: "PUT",
      body: { ...options.data },
      params: { ...options.params },
  ...
  globalThis.$Authorization = async (options: RequestOptions): Promise<ITokens | null> => {
    const res = (await $Post(config.LOGIN_URL, options)) as any;

    const { access, refresh } = res as ITokens;
    if (access && refresh) {
  ...
  globalThis.$RefreshToken = async function updateTokens(): Promise<ITokens | string | undefined> {
    if (token.refresh) {
      if (promise === null)
