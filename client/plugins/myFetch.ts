...
declare global {
  ...
  function $AuthorizationBase(options: RequestOptions): Promise<ITokens | null>
...
globalThis.$Post = async <T>(
  ...
  const authorizationBase = async <T>(
    apiUrl: string,
    options: RequestOptions
  ): Promise<T> => {
    return await $fetch(apiUrl, {
  ...
  globalThis.$AuthorizationBase = async (
    ...
    const res = (await authorizationBase(config.LOGIN_URL, options)) as any
