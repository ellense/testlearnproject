...
export const useReportStore = defineStore("ReportStore", {
  ...
  actions: {
    ...
    //       // Преобразование статуса перед сохранением данных
    //       const transformedInvoices = invoice.results.map(async invoice => {
    //         // Получение дополнительных данных из другого API
    //         this.setFilterValueInvoices('doc_id', invoice.venddoc);
    //         await this.getInvoicesFromAPIWithFilter();
    //         return {
    //           ...invoice,
    //           invoice_status: getStatusText(invoice.invoice_status),
    //           invoice_number: this.invoicesFull[0].invoice_number,
    //           invoice_date: this.invoicesFull[0].invoice_date,
    //           purch_number: this.invoicesFull[0].purch_number,
    //           purch_date: this.invoicesFull[0].purch_date,
    //         };
    //       });
    //       const resolvedInvoices = await Promise.all(transformedInvoices);
    //       allInvoices = allInvoices.concat(resolvedInvoices);
    //       totalPages = Math.ceil(invoice.count / this.$state.countRowTable);
    //       nextPage++;
    //     }
    //     console.log("Все данные о invoice for graphic", allInvoices);
    //     this.$state.invoices = allInvoices;
    //   } catch (error) {
    //     console.error("Ошибка при получении данных invoice for graphic", error);
    //     return Promise.reject(error);
    //   }
    // },

    // async getInvoicesFromAPIWithFilter(page?: number) {
    //   await INVOICE.getInvoicesFullListForReport({
    //     page_size: this.$state.countRowTable,
    //     page,
    //     doc_id: this.$state.filterValueInvoice?.doc_id,
    //   })
    //     .then((dataInvoice) => {
    //       console.log('Получены данные накладных:', dataInvoice);
    //       this.$state.invoicesFull = dataInvoice.results;
    //       this.$state.pagination = {
