<template>
  <el-dialog v-model="store.dialogFormExInvoiceVisible" title="Выбор исключенных накладных для КУ" close-on-click-modal
    ...
    <h4>Наименование поставщика: <span style="font-weight: 400;">{{ store.kuIdVendorNameExInvoice }}</span></h4>
...
const { getExInvoiceAll, pagination, countRowTable } = storeToRefs(store2);
...
const paginationChange = (page: number) => {
  ...
  loading.value = true;
  store2.getInvoicesFromAPIWithFilter(page);
  loading.value = false;
};

const searchQuery = ref('');
watch(searchQuery, (newValue: string) => {
  store2.performSearchOfInvoice(newValue);
});
...
const changeDateRange = (newDateRange: Date[]) => {
  if (newDateRange && Array.isArray(newDateRange) && newDateRange.length === 2) {
    ...
    if (!startDate || !endDate) {
      store2.removeFilterExInvoice('start_date');
      store2.removeFilterExInvoice('end_date');
    } else {
      const startFormatted = formatDate(startDate);
      const endFormatted = formatDate(endDate);
      store2.setFilterExInvoice('start_date', startFormatted);
      store2.setFilterExInvoice('end_date', endFormatted);
    }
